...
const checkout = async (req, res) => {
  ...
  try {
    ...
    const orderNumber = String(orderCountResult.rows[0].next_num).padStart(5, '0');
    ...
    const orderResult = await client.query(`
      INSERT INTO fact_orders (
        ...
        subtotal, discount_amount, discount_id, tax_amount, shipping_fee,
    ...
    for (const item of orderItems) {
      await client.query(`
        INSERT INTO fact_order_items (
          ...
          cost_price, subtotal, discount_amount, tax_amount, total
      ...
      await client.query(`
        INSERT INTO fact_inventory_transactions (
          variant_id, store_id, date_key, transaction_type_id,
          quantity_change, reference_type, reference_id,
          notes, created_by
        ) VALUES (
          $1, $2, $3,
          (SELECT id FROM subdim_inventory_transaction_types WHERE code = 'SALE'),
          $4, 'order', $5, 'Bán hàng POS', $6
        )
      `, [
        item.variant_id,
        store_id,
    ...
    if (discount_id) {
      await client.query(`
        INSERT INTO fact_discount_usages (
          discount_id, order_id, customer_id, amount_applied
        ) VALUES ($1, $2, $3, $4)
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        name: product.variant_name ? `${product.product_name} - ${product.variant_name}` : product.product_name,
        price: parseFloat(product.price || 0),
        stock: parseInt(product.stock || 0),
        reserved: parseInt(product.reserved || 0),
        available: parseInt(product.stock || 0) - parseInt(product.reserved || 0)
      }
    });
...
const saveDraft = async (req, res) => {
  try {
    ...
    `, [today.toISOString().split('T')[0]]);
    const draftNumber = String(draftCountResult.rows[0].next_num).padStart(3, '0');
    ...
    for (const item of items) {
      ...
      `, [
        ...
        ((item.unit_price || 0) * item.quantity) - (item.discount_amount || 0)
      ]);
    }
  ...
  try {
    ...
    const result = await db.query(`
      ...
      GROUP BY o.id, o.order_code, o.created_at, o.subtotal, 
               o.discount_amount, o.final_amount, o.notes,
    ...
    return res.status(200).json({
      ...
      data: {
        receipt: {
          ...
          payment: {
            method: paymentMethodNames[order.payment_method] || order.payment_method,
            amount_received: parseFloat(order.paid_amount || 0),
            change: parseFloat(order.change_amount || 0)
          },
          notes: order.notes
        }
      }
...
const getPaymentMethods = async (req, res) => {
  try {
    const methods = [
      ...
      { id: 'bank', name: 'Chuyển khoản', icon: 'fa-building-columns', enabled: true },
